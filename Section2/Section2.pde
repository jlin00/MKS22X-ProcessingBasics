...
void gasketH(int current, int levels, float v1x, float v1y, float v2x, float v2y, float v3x, float v3y) {
  if (current == 0) {
    fill(255, 0, 0); //draw the first triangle, which is red 
    background(255, 255, 255); //white background
    triangle(v1x,v1y,v2x,v2y,v3x,v3y);
  }
  if (current < levels){
    fill(255, 255, 255); //remove the middle triangle, aka draw a white triangle 
    triangle((v1x + v2x)/2, (v1y + v2y)/2, (v1x + v3x)/2, (v1y + v3y)/2, (v2x + v3x)/2, (v2y + v3y)/2); //middle triangle 
    //top, left, right triangles, which remain red 
